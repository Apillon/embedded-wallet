...
function COSEPublicKey_decode(buf: ArrayBufferLike): COSEPublicKey_EC {
  ...
  if (kty == 2) {
    ...
    if (
      !(ret.alg == -7 && ret.crv == 1) && // ES256 + P-256 (NIST)
      !(ret.alg == -8 && ret.crv == 6)
    ) {
      // EdDSA + Ed25519
...
const asn1_sig_schema = new asn1js.Sequence({
  name: 'sig',
  value: [
    new asn1js.Integer({
      name: 'r',
...
export async function credentialGet(credentials: Uint8Array[], challenge?: Uint8Array) {
  ...
  const authed = (await navigator.credentials.get({
    publicKey: {
      allowCredentials: credentials.map(_ => {
        return { id: _, type: 'public-key' } as PublicKeyCredentialDescriptor;
      }),
      challenge,
    },
  ...
  const result = decodedSignature.result as unknown as { r: asn1js.Integer; s: asn1js.Integer };
